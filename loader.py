...
class room:
  ...
  def open_room(self,name):
    if self.firstrun == True:
      ...
      unopened = []
      for names in os.listdir(sys.path[0]+'/house/'+name.lower()):
      ...
      if len(unopened) != 0: self.current = random.choice(unopened)
        ...
        self.current = 'end'
    script = []
    with open(sys.path[0]+'/house/'+name.lower()+'/'+self.current) as choice: script = choice.readlines()
    th = len(script)+2
    tw = max(len(self.current),len(max(script,key=len)))
    self.textarea = curses.newpad(th,tw)
    y,x = 0,0
    for i in range(len(script)+1):
      if i < len(script): words = script[i].rstrip().split(' ')
      else: words = self.current.rstrip().split(' ')
      for j in words:
        if j.lower() in self.specials:
          self.textarea.addstr(y,x,j,curses.color_pair(1))
          self.current_specials.append([y,x,j])
        else: self.textarea.addstr(y,x,j)
        x += len(j)+1
      y += 1
      x = 0
    with open('transcript','a') as opened: opened.write(self.current+'\n')
    with open('log','w') as master: master.write(name.lower()+'\n'+self.current)
